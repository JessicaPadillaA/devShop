...
export default function ShirtPage() {
    ...
    {allShirts.map((shirt) => (
        (shirt.prodCode === prodCodeLocal) ? (
            <div className="details__product" key={shirt.prodCode}>
                <div className="details__product__image">
                    <img src={shirt.imagePath} alt="Imagen" className="product-details__image" />
                </div>

                <div className="details__product__info">
                    <h3 className="details__product__name">Producto: {shirt.prodName}</h3>
                    <h3 className="details__product__description">Descripción: {shirt.description}</h3>
                    <h3 className="details__product__size">Talla: {shirt.size}</h3>
                    <h3 className="details__product__price">Precio: ${shirt.price}</h3>
                    <h3 className="details__product__brand">Marca: {shirt.brand}</h3>
